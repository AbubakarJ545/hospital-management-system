...
const PatientFormModal = ({ isOpen, onClose, onPatientAdded, preselectedDoctor = null }) => {
  const [departments, setDepartments] = useState([]);
  const [allDoctors, setAllDoctors] = useState([]);
  const [filteredDoctors, setFilteredDoctors] = useState([]);
  const [formData, setFormData] = useState({
  ...
  useEffect(() => {
    if (preselectedDoctor && isOpen) {
      setFormData(prev => ({
        ...prev,
        departmentId: preselectedDoctor.department?._id || preselectedDoctor.department,
        doctorId: preselectedDoctor._id
      }));
      
      // Filter doctors for the selected department
      const filtered = allDoctors.filter(
        (doctor) => doctor.department?._id === preselectedDoctor.department?._id || 
